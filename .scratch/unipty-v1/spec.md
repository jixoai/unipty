...
- Concrete Backend packages use the uniform `@unipty/backend-*` namespace. The first-phase packages are `@unipty/backend-bun`, `@unipty/backend-node-pty`, and `@unipty/backend-deno-sigma__pty-ffi`; package names preserve the substrate provenance rather than reducing the last route to the runtime name Deno. On 2026-09-07 the second phase added `@unipty/backend-zigpty`, a second Node-route package over third-party `zigpty`: the route registry is keyed by substrate identity, never by runtime, and each registry entry pairs the package name with its exact backend id — evidence or metadata wearing an official package name but a foreign backend id never counts toward that route's coverage.
- The Bun route is based on the runtime-native `Bun.Terminal`, the Node routes on third-party `node-pty` and third-party `zigpty` (Zig-built NAPI prebuilds, hard native gate, text-only substrate writes), and the last route on third-party `@sigma/pty-ffi` over Rust `portable-pty`. Deno is runtime metadata for `@unipty/backend-deno-sigma__pty-ffi`, not its complete implementation identity. Package ownership, substrate provenance, and runtime ownership remain separate claims.
...
- Conformance CI runs each Backend through its installed public package surface and emits one machine-readable evidence artifact only after the complete suite passes. A deterministic aggregation job validates record schema, package/metadata identity, suite version, tested commit, tuple normalization, uniqueness, and the release workflow's required route coverage; it then emits one catalog JSON artifact with stable ordering. Release route coverage requires at least one native passing tuple for each official route — `@unipty/backend-node-pty`, `@unipty/backend-zigpty`, `@unipty/backend-bun`, and `@unipty/backend-deno-sigma__pty-ffi`; only the exact passing tuples become `verified`.
...
- Terminal Stream completion observes Terminal transport EOF (or reports a transport read error), while Process Exit Result independently observes child-process completion as `{ exitCode: number | null; signal: string | null }`. EOF does not synthesize an exit result, process exit does not force a clean stream completion, and cancelling a stream does not cancel process-exit observation. Substrates that expose no transport-EOF event at all (zigpty; `Bun.Terminal` when its transport callback does not fire) force a declared synthesis: the adapter completes the output source from the exit observation, one macrotask deferred so trailing chunks still enqueue — a route-level limitation that must be documented (post-leader-death output from descendants still holding the slave is cut, and a transport read error is indistinguishable from clean EOF) rather than silently presented as kernel EOF.
...
- 2026-09-07: The official route set gained its fourth member `@unipty/backend-zigpty` (substrate `zigpty@0.2.1`, targets `node`). The catalog route registry was re-keyed from runtime tokens to route identities so multiple routes may share one runtime; the release gate covers every official route independently. The adapter pins three substrate laws verified by probe: readiness rejects `hasNative === false` (the pipe pseudo-PTY fallback is never entered), Endpoint `close()` defers the substrate close (which SIGHUPs a live child) until the exit observation settles, and output-source completion is synthesized from `exited` one macrotask later because the substrate exposes no transport-EOF event (Bun-route precedent). Substrate `write` is string-only, so Endpoint input is text-native with the Backend-owned `writeDecode` byte-input convenience in both encoding modes.
