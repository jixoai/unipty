...
| Windows target                         | ✓ ConPTY*              | ✗ fail-closed                             | ✓ ≥ 1.3.14*                   | ✗                            | *evidence-gated (see the catalog); the zigpty engine itself ships Windows prebuilds, but this route refuses readiness on win32 because the substrate's `pause()`/`resume()` are no-ops there                                                                              |
