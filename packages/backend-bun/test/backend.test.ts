...
describe("createBunBackend factory", () => {
  test("returns a ready backend with the default queue policy", async () => {
  ...
  test("honors the Backend-owned writeQueueBytes tuning option", async () => {
    const backend = await createBunBackend({ writeQueueBytes: 4096 });
    expect(backend.writeQueueBytes).toBe(4096);
  }, 10_000);

  test("rejects malformed writeQueueBytes with invalid-argument", async () => {
  ...
  test("rejects a runtime without Bun.Terminal with unsupported", async () => {
  ...
  test("the support-floor predicate matches the substrate release history", () => {
  ...
  test("dispose resolves immediately (runtime-owned substrate) and is reusable", async () => {
    const backend = await createBunBackend();
...
describe("Bun Backend Endpoint over a real PTY", () => {
  test("spawns /bin/echo, emits native byte chunks, and observes exit code 0", async () => {
    const backend = await createBunBackend();
  ...
  test("accepts native byte input and (defense-in-depth) encoded text input on /bin/cat", async () => {
    const backend = await createBunBackend();
    ...
    try {
      ...
      // Byte-native path.
      expect(
        endpoint.write({ kind: "bytes", bytes: new TextEncoder().encode("unipty-ping\n") }),
      ).toBe(true);
  ...
  test("initial geometry reaches the child", async () => {
    const backend = await createBunBackend();
  ...
  test("resize is accepted and the exit observation still works", async () => {
    const backend = await createBunBackend();
    const endpoint = backend.spawn(launch(["/bin/cat"]));
    try {
      endpoint.resize(120, 40);
      expect(
        endpoint.write({ kind: "bytes", bytes: new TextEncoder().encode("still-writable\n") }),
      ).toBe(true);
  ...
  test("observes a non-zero child exit code", async () => {
    const backend = await createBunBackend();
  ...
  test("terminate() requests SIGTERM on a sleeping child and is idempotent", async () => {
    const backend = await createBunBackend();
  ...
  test("close() closes the transport only: no child signal, I/O surfaces reject, exit observation survives", async () => {
    const backend = await createBunBackend();
    const endpoint = backend.spawn(launch(["/bin/sleep", "30"]));
    try {
      endpoint.close();
  ...
  test("close() completes the active output source cleanly", async () => {
    const backend = await createBunBackend();
    const endpoint = backend.spawn(launch(["/bin/cat"]));
  ...
  test("reports a synchronous typed failure for an unlaunchable executable", async () => {
    const backend = await createBunBackend();
  ...
  test("spawn rejects an empty argv defensively", async () => {
    const backend = await createBunBackend();
...
describe("bounded pending-write queue policy", () => {
  test("false at the soft mark, whole-value backpressure rejection at the hard bound, drain recovers", async () => {
    ...
    try {
      ...
      let saturated:
        | { code?: string; details?: { pendingBytes?: number; valueBytes?: number } }
        | undefined;
      try {
  ...
  test("drain() rejects with closed when the endpoint closes before readiness recovers", async () => {
    const backend = await createBunBackend({ writeQueueBytes: 4096 });
    const endpoint = backend.spawn(launch(["/bin/cat"]));
    try {
