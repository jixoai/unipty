...
import { type GlobOptions, globToRegExp } from "../../path/1.1.6/glob_to_regexp.ts";
import { joinGlobs } from "../../path/1.1.6/join_globs.ts";
import { isGlob } from "../../path/1.1.6/is_glob.ts";
import { isAbsolute } from "../../path/1.1.6/is_absolute.ts";
import { resolve } from "../../path/1.1.6/resolve.ts";
import { SEPARATOR_PATTERN } from "../../path/1.1.6/constants.ts";
...
export async function* expandGlob(
  ...
  const shouldInclude = (path: string) => !excludePatterns.some((p) => p.test(path));
