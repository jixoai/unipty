...
function extname$1(path) {
	...
	let matchedSlash = true;
	let preDotState = 0;
	if (path.length >= 2 && path.charCodeAt(1) === 58 && isWindowsDeviceRoot(path.charCodeAt(0))) start = startPart = 2;
...
function resolve$1(...pathSegments) {
	...
	for (let i = pathSegments.length - 1; i >= -1; i--) {
		let path;
		if (i >= 0) path = pathSegments[i];
		else if (!resolvedDevice) path = cwd("Resolved a drive-letter-less path without a current working directory (CWD)");
		else {
...
function throwIfNotDirectory(fileInfo) {
	if (!fileInfo.isDirectory) throw new Error(`Failed to ensure directory exists: expected 'dir', got '${getFileInfoType(fileInfo)}'`);
}
//#endregion
...
new RegExp(["[\\u001B\\u009B][[\\]()#;?]*(?:(?:(?:(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]+)*|[a-zA-Z\\d]+(?:;[-a-zA-Z\\d\\/#&.:=?%@~_]*)*)?\\u0007)", "(?:(?:\\d{1,4}(?:;\\d{0,4})*)?[\\dA-PR-TXZcf-nq-uy=><~]))"].join("|"), "g");
//#endregion
...
function baseUrlToFilename(url) {
	...
	switch (protocol) {
		...
		case "data":
		case "blob": break;
		default: throw new TypeError(`Don't know how to create cache name for protocol: ${protocol}`);
	}
	return join(...out);
}
/**
...
function homeDir() {
	switch (Deno.build.os) {
		case "windows": return Deno.env.get("USERPROFILE");
		case "linux":
		case "darwin":
		case "freebsd":
		case "netbsd":
		...
		case "solaris":
		case "illumos":
		case "android": return Deno.env.get("HOME");
...
function getCrossOption(record) {
	...
	if (ALL_OSS.some((os) => os in record)) {
		...
		if (subrecord && typeof subrecord === "object" && ALL_ARCHS.some((arch) => arch in subrecord)) return subrecord[Deno.build.arch];
		else return subrecord;
	}
	if (ALL_ARCHS.some((arch) => arch in record)) {
		const subrecord = record[Deno.build.arch];
		if (subrecord && typeof subrecord === "object" && ALL_OSS.some((os) => os in subrecord)) return subrecord[Deno.build.os];
		else return subrecord;
	}
}
/**
...
function createDownloadURL(options) {
	...
	for (const key in options.extensions) {
		const os = key;
		if (options.extensions[os] !== void 0) options.extensions[os] = options.extensions[os].replace(/\.?(.+)/, "$1");
	}
	let url;
	if (options.url instanceof URL) url = options.url;
		...
		if (tmpUrl === void 0) throw new TypeError(`An URL for the "${Deno.build.os}-${Deno.build.arch}" target was not provided.`);
	...
	if ("name" in options && !Object.values(options.extensions).includes(extname(url.pathname))) {
		...
		if (options.name === void 0) throw new TypeError(`Expected the "name" property for an automatically assembled URL.`);
...
async function ensureCacheLocation(location = "deno") {
	if (location === "deno") {
		...
		if (dir === void 0) throw new Error("Could not get the deno cache directory, try using another CacheLocation in the plug options.");
		location = join(dir, "plug");
	} else if (location === "cache") {
		const dir = cacheDir();
		if (dir === void 0) throw new Error("Could not get the cache directory, try using another CacheLocation in the plug options.");
		location = join(dir, "plug");
	} else if (location === "cwd") location = join(Deno.cwd(), "plug");
	else if (location === "tmp") location = await Deno.makeTempDir({ prefix: "plug" });
	else if (typeof location === "string" && location.startsWith("file://")) location = fromFileUrl(location);
	else if (location instanceof URL) {
		if (location?.protocol !== "file:") throw new TypeError("Cannot use any other protocol than file:// for an URL cache location.");
	...
	if (!cached) {
		...
		switch (url.protocol) {
			...
			case "file:":
				...
				break;
			default: throw new TypeError(`Cannot fetch to cache using the "${url.protocol}" protocol`);
		}
...
async function instantiate(libPath) {
	...
	LIBRARY = await dlopen({
		name: "pty",
		url: isLocalDev ? Deno.env.get("RUST_LIB_PATH") : `https://github.com/sigmaSd/deno-pty-ffi/releases/download/${version}`,
...
function libName() {
	switch (Deno.build.os) {
		case "linux": return `libpty_${Deno.build.arch}.so`;
		case "darwin": return `libpty_${Deno.build.arch === "x86_64" ? "x86_64" : "arm64"}.dylib`;
		case "windows": return "pty.dll";
...
var Pty = class {
	...
	*/
	read() {
		...
		switch (status) {
			...
			default: throw new Error(`Pty read returned unexpected status: ${status}`);
		}
	}
	/**
	...
	*/
	readBytes() {
		...
		switch (status) {
			...
			default: throw new Error(`Pty readBytes returned unexpected status: ${status}`);
		}
	}
	/**
	...
	*/
	getSize() {
		...
		const errorPtrBuf = /* @__PURE__ */ new BigUint64Array(1);
		if (getLibrary().symbols.pty_get_size(this.#ptr, resultDataPtrBuf, resultLenBuf, errorPtrBuf) === -1) {
			const errorMsg = readErrorAndFree(getLibrary(), errorPtrBuf);
	...
	*/
	resize(size) {
		...
		const errorPtrBuf = /* @__PURE__ */ new BigUint64Array(1);
		if (getLibrary().symbols.pty_resize(this.#ptr, sizeDataPtr, BigInt(sizeData.length), errorPtrBuf) === -1) {
			const errorMsg = readErrorAndFree(getLibrary(), errorPtrBuf);
	...
	*/
	get readable() {
		if (!this.#ptr) return new ReadableStream({ start(controller) {
	...
	*/
	get writable() {
		if (!this.#ptr) return new WritableStream({
			start(controller) {
				controller.error(/* @__PURE__ */ new Error("Pty is closed."));
			},
			write() {
