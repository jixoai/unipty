...
describe("unipty.metadata default export", () => {
  ...
  it("identifies as third-party zigpty, never a native Node runtime API", () => {
    ...
    expect(metadata.provenance?.substrate).toContain("zigpty");
    expect(metadata.backend.id).toBe("zigpty");
  });
...
describe("metadata side-effect freedom (built artifact)", () => {
  it("imports without loading a native addon or creating any pty", () => {
    ...
    expect(report.backend).toEqual({ id: "zigpty", factoryExport: "createZigptyBackend" });
  });
});
