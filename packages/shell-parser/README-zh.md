...
结果为 `argv` 时，文本在**词法上**是单个简单命令、全部字面词（保留引号语义与
空参数）——一个可直接交给 `unipty.spawn(argv)` 的启动候选。可执行文件解析
（PATH、内建命令、函数、别名）仍是**你**的决定；解析器绝不对命令名声称进程
启动等价性。结果为 `script` 时，文本携带 shell 语义（管道、重定向、展开、替
换、glob、波浪号、后台、赋值前缀、locale 字符串、复合语句……），**你**必须
显式接受该 shell 策略后才能启动任何东西。

## 分类策略

- `argv`——恰好一个简单命令；每个词在静态引号处理后都是字面量；没有展开、
  替换、glob、波浪号、转义、locale 字符串（`$"..."`）、含 NUL 的 ANSI-C 字
  符串、重定向、赋值前缀、操作符或复合构造。
