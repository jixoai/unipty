...
export const zh: SiteContent = {
      ...
      rows: [
        {
          ...
          notes: '第三方原生插件，随包附带预构建二进制。Node 没有原生 PTY API；本路由如实封装生态标准底层——绝不宣称适配的是 Node 运行时原生 API。',
        },
        {
          pkg: '@unipty/backend-bun',
          runtime: 'Bun',
          substrate: 'Bun.Terminal',
          notes: 'Bun 内建终端 API：Linux/macOS 自 Bun 1.3.13 起，Windows 经 ConPTY 自 1.3.14 起。支持是带版本的证据，不是笼统宣称。',
        },
        {
          pkg: '@unipty/backend-deno-sigma__pty-ffi',
          runtime: 'Deno',
          substrate: '@sigma/pty-ffi (Rust portable-pty)',
          notes: '仅以 npm 发布的包，构建期内嵌 @sigma/pty-ffi/noinit 闭包与目标动态库。需要显式 Deno FFI 权限。',
        },
      ],
  ...
  docs: {
    ...
    acquisition: {
      ...
      summary: '手动导入是一等路径且永远不会消失；AutoResolve 是其上的便利层；纯解析与检查保持无副作用。',
      sections: [
        {
      ...
      rows: [
        {
          ...
          runtime: 'Node',
          substrate: 'node-pty via @lydell/node-pty prebuilds',
          notes: '第三方原生插件，随包附带预构建二进制。Node 没有原生 PTY API；本路由如实封装生态标准底层，而不是假装不然。',
        },
        {
          pkg: '@unipty/backend-bun',
          runtime: 'Bun',
        ...
        },
        {
          ...
          runtime: 'Deno',
          substrate: '@sigma/pty-ffi (Rust portable-pty)',
          notes: '仅以 npm 发布的包，构建期内嵌 @sigma/pty-ffi/noinit JavaScript 闭包与目标动态库。需要显式 Deno FFI 权限；没有默认下载或缓存。',
        },
      ],
    },
    metadata: {
