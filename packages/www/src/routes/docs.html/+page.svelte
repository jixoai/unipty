<!--
  ...
  scaffold's SSR-stable chrome snippet from +page.ts `toc` data, and the
  shell grid reserves the rail column (desktop) / toc bar row (mobile).
  Scroll reveal rides the theme's pure CSS [data-reveal] hooks; CardGrid
  owns its children's entrance (cards carry no data-reveal).
-->
<script lang="ts">
...
<!-- Reading rail: authored in the layout's chrome snippet (SSR-stable) from
     this page's `toc` data; the shell grid reserves the rail column. -->
